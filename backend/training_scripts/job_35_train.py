#!/usr/bin/env python3
"""
LoRA Training Script for ascssdfsdfdffff
...
def send_output_to_frontend(job_id, message):
    """Send real-time output to frontend via Socket.IO"""
    try:
        requests.post(f'http://localhost:5000/api/training-jobs/{job_id}/output', 
                    json={'output': message, 'timestamp': datetime.now().isoformat()}, 
                    timeout=1)
    except:
...
def main():
    try:
        job_id = 35
        logger.info("🚀 Starting LoRA training for ascssdfsdfdffff")
        send_output_to_frontend(job_id, "🚀 Starting LoRA training for ascssdfsdfdffff")
        
        # Job configuration
        
        # Model and data paths
        base_model = "Qwen/Qwen2.5-Coder-7B"
        train_data_path = "training_data/job_35/train.jsonl"
        val_data_path = "training_data/job_35/val.jsonl"
        output_dir = f"models/ascssdfsdfdffff_lora"
        
        # Load model with 4-bit quantization (more memory efficient)
        logger.info("📥 Loading base model...")
        send_output_to_frontend(job_id, f"📥 Loading base model: qwen2.5-coder:7b")
        ...
        trainer = Trainer(
            ...
            tokenizer=tokenizer,
            callbacks=[ProgressCallback(35)]
        )
        
        # Start training with proper trainer
        logger.info("🏃 Starting training...")
        send_output_to_frontend(job_id, "🏃 Starting LoRA fine-tuning training...")
